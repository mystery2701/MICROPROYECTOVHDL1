...
entity control is
    Port (
        ...
        b_dec, b_uni   : out integer range 0 to 9; 
        e_dec, e_uni   : out integer range 0 to 9  
    );
end control;
            ...
            case estado_actual is
                ...
                when OCUPADO_LIMITE =>
                    if sensor_persona = '0' then
                        ...
                        if base_d = 3 and base_u = 5 then
                            ...
                            if base_u = 9 then base_u <= 0; base_d <= base_d + 1;
                            else base_u <= base_u + 1; end if;
                        end if;
                    end if;
                ...
                when EXCESO_TIEMPO =>
                    ...
                    if sensor_persona = '0' then
                        ...
                        if extra_u = 9 then extra_u <= 0;
                            if extra_d /= 9 then extra_d <= extra_d + 1; end if;
                        else extra_u <= extra_u + 1; end if;
                    end if;
            end case;
        end if;
    end process;
    b_dec <= base_d; b_uni <= base_u; e_dec <= extra_d; e_uni <= extra_u;
end Behavioral;